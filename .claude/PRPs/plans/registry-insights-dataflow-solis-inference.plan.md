...
> **STATUS 2026-07-28 (v2) — ONE UNGATED ITEM LEFT.** Everything the report lists as remaining is **externally gated**: WS-D waits on the registry ER PRP Phase 5 seeding `registry_trade_taxonomy`; WS-C C.2/C.3 need live registry credentials (`otn_insights` role + `REGISTRY_DATABASE_URL`); WS-E.3/E.4 are capture-gated and Solis §12.3 calibration. **The only ungated engineering is WS-E.2** — lift warm-network (co-appearing bound GCs) from digest-only into a Solis scoring signal. If WS-E.2 ships or is dropped, this plan closes.
