...
**organizations.registry_ref_method / registry_linked_at** (integration seam, migration `0021`) — provenance for the binding: `registry_ref_method` is `ubi_exact`, `contractor_number_exact`, or `name_review_confirmed` (human-accepted name match); `registry_linked_at` is when it was last bound. A UBI and contractor number pointing at different entities is a conflict and stays unbound (never auto-merged). See `docs/integration-one-trade-network.md` Parts B & D.

**organizations.registry_identity_json** (migration `0022`) — cached PUBLIC identity snapshot from the registry contract view (`entity_id`, canonical name, ubi, contractor numbers, L&I phone, locality, `snapshot_at`), stamped at bind time. Display cache only — the registry stays the identity authority; refreshed on rebind.

**organization_identifiers** (migration `0023`) — contractor identifiers harvested from PUBLIC source evidence (`identifier_type` ∈ phone | ubi | contractor_number | email), stored normalized beside the raw form, each row carrying its `source_record_id` (no claim without a source; account-private artifacts excluded by policy). Parsers may emit optional `phone`/`ubi`/`contractorLicense` on `organizations[]` entries; the resolver persists them here and **backfeeds strong keys** onto `organizations.ubi`/`contractor_registration` when NULL (never overwrites). Feeds the phone-aware registry matching rules (`binding_name_phone`, `binding_phone_match`) and the strong-key registry link.

**registry_observations** (migration `0022`) — the reviewed observation queue on the One Trade Network seam, both directions. One row per deterministic suggestion: `observation_type` ∈ `binding_name_match` (accept ⇒ bind org, always human) | `phone_adoption` (accept ⇒ GLOBAL `public_business` contact — the paying accounts' bucket) | `alias_export` / `trade_export` (accept ⇒ nightly push to `registry_partner` staging). `trust_score` is a deterministic weighted sum stored beside `trust_components_json` (name/identifier/locality/role/corroboration/ruleHistory — `identifier`: 1 = evidence-backed phone agrees with L&I, 0 = contradicts, 0.5 = no evidence); `rule_key` buckets the per-rule accept history (Laplace-smoothed) that feeds the next pass — a non-binding rule with ≥10 human decisions at ≥95% accept auto-accepts with `decided_by='auto:rule-history'`. `dedupe_key` unique (decided rows never resurrected); `exported_at`/`applied_at` record what an accept did.
