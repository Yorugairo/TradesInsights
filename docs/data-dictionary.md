...
**registry_observations** (migration `0022`) — the reviewed observation queue on the One Trade Network seam, both directions. One row per deterministic suggestion: `observation_type` ∈ `binding_name_match` (accept ⇒ bind org, always human) | `phone_adoption` (accept ⇒ GLOBAL `public_business` contact — the paying accounts' bucket) | `alias_export` / `trade_export` (accept ⇒ nightly push to `registry_partner` staging). `trust_score` is a deterministic weighted sum stored beside `trust_components_json` (name/locality/role/corroboration/ruleHistory); `rule_key` buckets the per-rule accept history (Laplace-smoothed) that feeds the next pass — a non-binding rule with ≥10 human decisions at ≥95% accept auto-accepts with `decided_by='auto:rule-history'`. `dedupe_key` unique (decided rows never resurrected); `exported_at`/`applied_at` record what an accept did.
