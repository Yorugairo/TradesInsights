...
`brief:run` (premium) generates the **verified decision brief** — a short model-composed narrative layered on the deterministic decision memo — for the highest-scoring opportunities without one (or one explicit `--opportunity`). The model composes only from a fixed menu built from already-verified facts/inferences/context; a draft citing an unknown ref, or asserting a number/date not present verbatim in a cited item, is rejected (persisted `rejected`, surface falls back to the deterministic memo). Segment kind is derived from the cited refs so an inference is always hedged. Each attempt persists an account-scoped `model_runs.job_type='brief_draft'` (same budget/audit as extraction). Surfaced as a lead panel on the opportunity page and a one-line lead per digest item. Without model keys it records a visible blocked state.

`first-look` (premium) prints the **first-look advantage** per source×county: how often our earliest sighting of a project predates its `permit_issued` (publicly biddable) milestone, and — when early — the median days of lead. `permit_issued` is the public-visibility proxy (a permit surfaces on aggregator bid boards). Stated-date anchored, floored at 0, `--min` sample floor (default 20), test sources excluded. Also on the account **ROI page** (first-look table) and the customer-facing **`/app/pipeline`** view (`pipelineSummary` — the sourced→pursued→bid→won funnel + won/in-flight/OTN-influenced dollars + the lead-time headline, all reproduced from stored rows).
