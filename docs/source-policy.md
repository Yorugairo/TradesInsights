...
- Enabled: 2026-07-20 (WS-S; operator-local capture-fed). cadence `on_demand` ⇒ `schedulableSources` never schedules them in the datacenter; run via `pnpm source:run:operator-local` from the operator's in-region machine after a daily genuine-browser capture is staged into `fixtures/<key>/` (Akamai fingerprints the automated client, so `fetch()` 403s even in-region — never a bypass). Only the index (project + `TUM-YY-NNNN` case #, dated NOA/SEPA/NOD doc links) crosses — no party/PII, since the linked agenda/notice PDFs are not fetched (homeowner-PII gate); `organizations[]` empty, `normalizedStage` unknown.
