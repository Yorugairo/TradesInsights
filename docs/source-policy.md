...
- **UPDATE 2026-07-19 — REACHABLE IN-REGION FOR A GENUINE BROWSER ONLY.** From an in-region residential/municipal IP (AS14677 City of Tacoma) a normal browser loads both pages as an ordinary visitor. Golden fixtures captured: `fixtures/tumwater_development_review/drc-agendas.index.json` (80 DRC agenda PDFs + application forms + DOM map) and `fixtures/tumwater_sepa/noa-sepa.index.json` (59-row PROJECT/NOA/SEPA/Decision table, 14-row verbatim sample; `showpublisheddocument` PDFs + WA Ecology SEPA record links). Notice-PDF bodies not yet fetched (download-class; approved, pending the PDF→text pipeline).
- **Caveat (corrects the 2026-07-15 read):** Akamai here bot-fingerprints the CLIENT, not just the egress IP — a genuine browser passes in-region, but `curl`/automated fetchers still get HTTP 403 even from the same in-region IP. So the block was NOT purely datacenter-specific; both tumwater sources are genuine-visitor **capture-fed** (like PALS), and `fetch()` will dead-letter against the live gate. No bot/WAF bypass is used or implied.
- Re-verification path: the checklist passes from in-region via a genuine browser; no fixtures were fabricated.
- Enabled: 2026-07-20 (WS-S; operator-local capture-fed). cadence `on_demand` ⇒ `schedulableSources` never schedules them in the datacenter; run via `OTN_CAPTURE_DIR=<dir> pnpm source:run:operator-local` from the operator's in-region machine after a daily genuine-browser capture is staged into `$OTN_CAPTURE_DIR/<key>/` (Akamai fingerprints the automated client, so `fetch()` 403s even in-region — never a bypass; the adapter's `fetch()` reads the staged capture from `$OTN_CAPTURE_DIR`, never the golden fixtures dir). Only the index (project + `TUM-YY-NNNN` case #, dated NOA/SEPA/NOD doc links) crosses — no party/PII, since the linked agenda/notice PDFs are not fetched (homeowner-PII gate); `organizations[]` empty, `normalizedStage` unknown.
...
- **UPDATE 2026-07-19 — REACHABLE IN-REGION; PDF-ONLY confirmed.** From an in-region IP the ReportsView portal loads for an ordinary genuine-browser visitor and the six public reports enumerate (GUIDs in `fixtures/olympia_smartgov_reports/reports-catalog.json`). Invoking a report (`ReportsView.viewReport(guid,'Exago')`) mints a session-bound single-use `eid` and renders a server-generated **PDF** via `reports-new.smartgovcommunity.com/exago/ExecuteExport.aspx?eid=<runtimeId>&eit=0`. **Correction to the earlier note on this line:** although the Exago ENGINE advertises HTML/Excel/CSV/RTF, SmartGov's Olympia config returns HTTP 500 (server-side NullReferenceException) for every non-PDF `eit` (3/4/5/6) — the source is **PDF-only in practice**, so the adapter parses the PDF via positioned text (`packages/documents` `extractPdfTextItems`), not a clean tabular export. The `eid` is session-bound + single-execution, so this is genuine-visitor **capture-fed** (like PALS): captured `fixtures/olympia_smartgov_reports/permits-issued-last-30-days.pdf` (646 KB, 60 pages, 33 category sections, `Grand Total: 572` permits).
- Mitigations: `wa_sepa` covers Olympia lead-agency SEPA actions; Olympia was the top P1 permit-stage gap.
- Capture posture: PDF-only (non-PDF `eit` 500s); positional PDF parser (`olympia_smartgov_reports`) built 2026-07-19. Capture-fed — auto-`fetch()` dead-letters against the session-bound `eid` gate.
- Enabled: 2026-07-20 (WS-S; operator-triggered capture-fed). Cadence `on_demand` — never auto-scheduled by `schedulableSources`; a run consumes a staged genuine-visitor capture (`fixtures/olympia_smartgov_reports/permits-issued-last-30-days.pdf`, 646 KB). Live auto-`fetch()` still dead-letters against the session-bound `eid` gate by design — never bypassed.
...
- **Pierce PALS contractor API — CAPTURED IN-REGION 2026-07-19 (supersedes the "blocked at egress" note below):**
  From an in-region IP a real browser reaches PALS; the true gate is reCAPTCHA Enterprise v3 (invisible), NOT the IP — the app's own `$http` interceptor mints a per-request token, so a genuine Permit Search (type `applPermitId` → click Search) returns data as an ordinary visitor (no token-minting at scale, no bypass). **Endpoint correction:** `contractorInfo/:id` is keyed by the contractor LICENSE number (used only by the online-application form) and CANNOT map our permit IDs; the permit-keyed endpoint is `webApplPermitStatusHeader?applPermitId=<id>` (array of one: applicant name+phone+license, owner, parcel, status). **Terms gate:** PALS requires accepting a Terms of Use whose RCW 42.56.070(8) clause prohibits commercial use of *lists of individuals* — accepted 2026-07-19 with explicit owner authorization; a commercial-use compliance review is flagged before scaling. Golden set: `fixtures/pierce_pals_contractor/` (permits 1067557/1067558/1067549/1067513 + metadata; covers applicant=contractor, `**`-masked license, and applicant=owner/null-contractor cases). Enrichment stays Lookup-class (enrich known permits only, never a sole alert source). **Historical (now-superseded) attempt log:**
