...
## Wave 3 Phase 1 — batch 2 live findings (2026-07-23)

Second batch of the same plan. **Shipped:** generic `SocrataPermitsAdapter` (config-driven, the Socrata twin of `ArcgisPermitsAdapter`), **Everett** and **Spokane City** enabled, **Auburn** registered but `enabled:false`, and the Solis geography **distance ladder** (scoring v1.10.0). Live verification again corrected audit assumptions — the audit's own "recommended build order" row 2 does NOT survive contact:

| City | Live finding (2026-07-23) | Disposition |
|---|---|---|
| **Everett** ✅ | Socrata `3w3u-656c` ("Trakit Permits"). The earlier "1 confirming fetch" resolved RICH: applied/approved/issued/expired dates, **contractorname on ~94%** of rows, siteaddress, siteapn, jobvalue, GeoJSON `geocoded_column` (~97%). **No status column** — stage derives from date presence. The sibling dataset "Permits" (`7fiu-4gra`) returns EMPTY objects and is dead — do not use it. | **SHIPPED** `enabled:true`. **First genuine Snohomish-county source** (Everett is the county seat). |
| **Spokane City** ✅ | Audit URL was **wrong** (`…/Permit_WM_Dynamic2/MapServer/0` 404s — the service lives in the **`Permit/` folder**: `…/services/Permit/Permit_WM_Dynamic2/MapServer/0`). Layer 0 "Permit" is the only permit layer (1–3 are boundary/council overlays) ⇒ genuinely single-layer. OpenDate epoch, **100% point geometry + parcel**, clean 11-value Status vocabulary. **No contractor, no valuation, no units, no issued-date column.** | **SHIPPED** `enabled:true` as a demand/stage/geography source (not trades-matching). |
| **Auburn** ⛔ STALE | Schema is clean and rich as previously found (TENANTIMPROVEMENT/ALTERATIONS/BUILDING; contractor on ~81%), and the parser is proven against a real golden fixture. **BUT the ETL is FROZEN:** `max(applied) = 2025-02-25`, `rowsUpdatedAt = 2025-02-26` — and the SAME timestamp on **all three** Auburn permit datasets (`fted-8bve`, `fvjv-6bef`, `nfcm-it2j`). Auburn stopped publishing ~17 months ago. | **BACKLOG / registered `enabled:false`.** Enabling would immediately trip the stale source-health rule and serve 17-month-old data. One-flag flip if Auburn resumes. Re-check freshness first. |
| **Clark County** ⛔ NO CLEAN LAYER | Audit URL `gis.clark.wa.gov/arcgisfed2 …/PermitSitePlansTemp/MapServer/9` **404s**, and the name itself (`…Temp`) marks it non-authoritative. Service-directory sweep found no building-permit layer: `CommDev` is **empty**, `CodeAdministration` holds only `Fireworks_Use_Areas`, `UtilityPermits` holds only utility-permit services. Remaining folders are small-city services (Battle Ground, La Center, Ridgefield, Washougal, Yacolt), not the county. | **BACKLOG** — no authoritative county building-permit layer is publicly exposed. Needs a different access class (portal/Accela), not an ArcGIS config. |
| **Burien** ⛔ NOT DISCOVERABLE | `gis.burienwa.gov/arcgis/rest/services` returns an IIS 404 (no on-prem ArcGIS REST root). ArcGIS Hub dataset search for "Burien permit" returns **zero** results; the audit's `…cwpll` fragment does not resolve to a valid AGOL org. | **BACKLOG** — needs operator-side discovery of Burien's actual publishing surface before any adapter work. |

**Corrected remaining order:** (1) **OpenDataSoft** class → Vancouver; (2) **Excel-report** class → Snohomish County (note: Everett now covers the county's largest city, lowering this one's urgency); (3) **Kent** operator-local PDF (WAF-blocks datacenter IPs → genuine-browser capture lane); (4) **Renton** multi-layer union; (5) SeaTac CSV (operator confirms columns); (6) re-check **Auburn** freshness; (7) operator discovery for **Burien**/**Clark**.

**Standing lesson (now twice-confirmed):** the audit's field/URL lists are *approximate*. Every city must be re-verified live before shipping — batch 1 found Renton/Kitsap mis-scoped, batch 2 found a wrong Spokane URL, a dead Everett dataset, and a frozen Auburn feed. **Freshness is now part of verification, not just schema**: check `max(<applied>)` / `rowsUpdatedAt` before enabling any source.
