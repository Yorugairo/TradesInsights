 ...
 * bridge). The name is still emitted. Deliberately excludes a bare "CO"
 * (ambiguous token) and "TRUST" (family trusts are a residence-ownership vehicle)
 * so an individual is never misgated to a business and given a promoted address. */
const BUSINESS_OR_AGENCY_RE =
  /\b(LLC|L\.L\.C|INC|CORP|COMPANY|LTD|LP|LLP|PLLC|ROOFING|CONSTRUCTION|CONTRACTING|ENGINEERING|ELECTRIC(AL)?|PLUMBING|MECHANICAL|BUILDERS?|DEVELOPMENT|HOMES?|SERVICES?|SYSTEMS?|GROUP|ASSOCIATES|ENTERPRISES?|PARTNERS?|PROPERTIES|MANAGEMENT|HOLDINGS?|INVESTMENTS?|CAPITAL|REALTY|CITY|COUNTY|STATE|DISTRICT|DEPT|DEPARTMENT|PORT|UNIVERSITY|COLLEGE|SCHOOL|AUTHORITY|AGENCY|COMMISSION|ASSOCIATION|CHURCH|TRIBE|NATION|UTILITIES|WATER|SEWER|FIRE|HOSPITAL|MEDICAL|FOUNDATION)\b/i;
